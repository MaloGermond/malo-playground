...
function setup() {
	...
	page.define({ width: windowWidth - 15, height: windowHeight - 15, column: 15, row: 7, rowGap: 4, columnGap: 4 })
...
function draw() {
	...
	images.map(el => displayImage(el.src, page.getCell(el.x, el.y)[0]))
...
function loadImages(size) {
	for(let i = 0; i < size; i++) {
		images.push({
			...
			src: loadImage("https://picsum.photos/" + round(random(100, 200)))
		})
	}
}
