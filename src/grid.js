...
const grid = (function() {
	...
	function displayCells() {
		config.cells.map(cell => rect(cell.position.x, cell.position.y, config.columnWidth, config.rowHeight))
	}

	function getCell(x, y) {
		const result = config.cells.filter(cell => x > cell.position.x + config.x && x < cell.position.x + config.columnWidth + config.x && y > cell.position.y + config.y && y < cell.position.y + config.rowHeight + config.y)
		return result
	}
